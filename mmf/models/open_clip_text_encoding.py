...
@registry.register_model("open_clip_text_encoding")
class open_clip_text_encoding_class(BaseModel):
    ...
    def config_path(cls):
        ...
        return "configs/models/open_clip_text_encoding/defaults.yaml"
    ...
    def build(self):
        ...
        model,_,preprocess = open_clip.create_model_and_transforms(self.config.image_encoder.type, pretrained =self.config.image_encoder.params.pretrained, device=device)

        # image_features = model.encode_image(image)
        self.open_clip_preprocess = preprocess
        self.vision_module = model.encode_image
        ...
        self.tokenizer = open_clip.get_tokenizer(self.config.text_encoder.type)
        self.language_module = model.encode_text
        # self.meow_test = build_text_encoder(self.config.text_encoder_fake)
    ...
    def forward(self, sample_list):
        ...
        text_list = []
        for sample in sample_list["text"]:
            text_list.append(' '.join(sample[1:-1]))
